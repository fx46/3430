...
class AutoAdaptiveQueueTest(unittest.TestCase):
    ...
    def test_b_EnqueueDequeue(self):
        ...
        oldSize = self.autoQueue3.size()
        for i in range(oldSize):
            self.autoQueue3.dequeue()

        self.assertEqual(self.autoQueue3.size(), self.autoQueue3.queueSize)

        oldSize = self.autoQueue3.size()
        for i in range(oldSize):
            self.autoQueue3.dequeue()
        self.assertRaises(ValueError, self.autoQueue3.dequeue())
